<template>
	<view class="uni-rate">
		<view :key="index" :style="{ marginLeft: margin + 'px' }" @click="_onClick(index)" class="uni-rate__icon" v-for="(star, index) in stars">
			<icon :color="color" :size="size" :type="isFill ? 'loveb' : 'lovea'" />
			<!-- #ifdef APP-NVUE -->
			<view :style="{ width: star.activeWitch.replace('%','')*size/100+'px'}" class="uni-rate__icon-on">
				<icon style="text-align: left;" :color="activeColor" :size="size" type="loveb" />
			</view>
			<!-- #endif -->
			<!-- #ifndef APP-NVUE -->
			<view :style="{ width: star.activeWitch}" class="uni-rate__icon-on">
				<icon :color="activeColor" :size="size" type="loveb" />
			</view>
			<!-- #endif -->
		</view>
	</view>
